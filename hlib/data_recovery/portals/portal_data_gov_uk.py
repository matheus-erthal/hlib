...
class PortalDataGovUK(Portal):
    """
    Implementação para data.gov.uk (UK)
    """
    
    def __init__(self, **config):
        super().__init__(**config)
        self.adapter = CkanAdapter("https://ckan.publishing.service.gov.uk")

    async def search(self, query: str) -> List[Dataset]:
        results = []
        async with self.adapter as ad:
            if not await ad.connect():
                print("Could not connect to data.gov.uk")
                return []
    ...
    def _map_package_to_dataset(self, pkg: dict) -> Dataset:
        """
        Mapeia CKAN UK -> Hipólita Dataset.
        """
        resources = []
